...
async function WaitForItToWork_DocCauHinh(client, timeoutMs = 120000) {
    return new Promise((resolve) => {
        let rawBuffer = "";
        ...
        const onData = (data) => {
            rawBuffer += Buffer.isBuffer(data) ? data.toString("utf8") : String(data);

            const value = extractValueInParentheses(rawBuffer);
            if (value !== null && value !== "") {
                cleanup();
                resolve(value);
            }
