...
const post = async (req, res, next) => {
    ...
    upload(req, res, async function (err) {
        ...
        const mataikhoan = parseInt(req.body.mataikhoan);
            ...
            case 'xml':
                ...
                if (table) {
                    try {
                        ...
                        // Bước 2: Kiểm tra khách hàng sau khi insert
                        var tableJson = await ThucHienKhaiBao(code, type, mataikhoan, dieukienkhaibao, ip, conn);
...
// Hàm chuyển đổi file XML thành "table" dữ liệu
...
async function ThucHienKhaiBao(code, type, mataikhoan, dieukienkhaibao, ip, conn) {
    return conn.execute(`BEGIN PKG_KHAIBAOKHACHHANG_XML_EXCEL.P_KHAIBAO_DS_KH_XML(:code,:dieukienkhaibao,:type,:mataikhoan,:ip,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        code: {
