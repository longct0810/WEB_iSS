    ...
    $("#titlecongto").html(`
        <div class="meter-header">
            <div class="meter-title">
                ⚙️ Cấu hình công tơ: <span>${node.socongto}
            </div>
...
function f_loaisukien3Pha() {
    ...
    const eventConfigs = {
        ...
        METER_TIME_SETTING: {
            ...
            displayName: "Thời gian công tơ",
            params: [
                {
                    id: "meterTimeSetting",
                    viName: "Thời gian công tơ",
        ...
        FUTURE_TOU_SEASON_ACTIVE_TIME: {
            group: "Cài đặt biểu giá tương lai (dự phòng)",
            displayName: "Cài thời gian kích hoạt của từng mùa",
            displayName: "Cài thời gian kích hoạt của từng mùa",
            params: [
                {
        ...
        FUTURE_TARIFF_ACTIVE_DATETIME: {
            group: "Cài đặt biểu giá tương lai (dự phòng)",
            displayName: "Cài thời gian kích hoạt biểu giá tương lai",
            params: [
                {
                    id: "futureTariffActiveDateTime",
                    viName: "Cài thời gian kích hoạt biểu giá tương lai",
                    enName: "Future tariff active datetime",
                    obis: "0.0.13.6.0",
                    unit: "DateTime",
                    fomat: "YYMMDDHHmm"
                }
        ...
        SPECIAL_DAY_SETTING: {
            group: "Cài ngày đặc biệt",
            displayName: "Ngày đặc biệt",
            params: [
                {
                    id: "specialDaySetting",
                    viName: "Cài đặt ngày đặc biệt",
                    enName: "Special day setting",
                    obis: "0.0.13.9.1",
                    unit: "Date",
                    fomat: "DDMM"
                }
        ...
        SPECIAL_DAY_TARIFF_TIME: {
            group: "Cài ngày đặc biệt",
            displayName: "Cài bảng thời gian chuyển biểu giá của ngày đặc biệt",
            params: [
                {
                    id: "specialDayTariffTime",
                    viName: "Cài bảng thời gian chuyển biểu giá của ngày đặc biệt",
                    enName: "Special day tariff time",
                    obis: "0.0.13.9.2",
                    unit: "Time",
                    fomat: "HHmm tariff"
                }
    ...
    function renderParams(eventKey) {
        ...
        `;
        ...
        else if (eventKey === "TOU_TARIFF_SEASON_1") {
            div.innerHTML = `
        ...
        else if (eventKey === "TOU_TARIFF_SEASON_2") {
            div.innerHTML = `
                <div class="tariff-season-box">
        ...
        else if (eventKey === "TOU_TARIFF_SEASON_3") {
            div.innerHTML = `
                <div class="tariff-season-box">
        ...
        else if (eventKey === "TOU_TARIFF_SEASON_4") {
            div.innerHTML = `
                <div class="tariff-season-box">
            ...
            div.innerHTML = renderFutureSeasonActiveTime(cfg);
            initFutureSeasonActiveTime();
            return;
        }

        else if (FUTURE_TARIFF_SEASON_META[eventKey]) {
            const meta = FUTURE_TARIFF_SEASON_META[eventKey];

            div.innerHTML = renderFutureTariffSeason(meta);
            initFutureTariffSeason(meta);
            return;
        }
        else if (eventKey === "FUTURE_TARIFF_ACTIVE_DATETIME") {
            div.innerHTML = renderFutureTariffActiveDateTime();
            initFutureTariffActiveDateTime();
            return;
        }
        else if (eventKey === "SPECIAL_DAY_SETTING") {
            div.innerHTML = renderSpecialDaySetting(cfg);
            initSpecialDaySetting();
            return;
        }
        else if (eventKey === "SPECIAL_DAY_TARIFF_TIME") {
            div.innerHTML = renderSpecialDayTariffTime();
            initSpecialDayTariffTime();
            return;
        }
...
function f_loaisukien1Pha() {
    ...
    const eventConfigs = {
        ...
        SPECIAL_DAY_TARIFF_TIME: {
            group: "Cài ngày đặc biệt",
            displayName: "Thời gian chuyển biểu giá của ngày đặc biệt",
            params: [
                {
                    id: "specialDayTariffTime",
                    viName: "Thời gian chuyển biểu giá của ngày đặc biệt",
                    enName: "Special day tariff time",
                    obis: "0.0.13.9.2",
                    unit: "Time",
                    fomat: "HHmm tariff"
    ...
    function renderParams(eventKey) {
        ...
        else if (TARIFF_SEASON_META[eventKey]) {
...
function renderMeterTimeSetting() {
    return `
        <div class="meter-time-box">
            ...
            <div class="meter-time-mode">
                <label>
                    <input type="radio" name="meterTimeMode" value="server" checked>
                    Thời gian server
                </label>

                <label>
                    <input type="radio" name="meterTimeMode" value="custom">
                    Thời gian tùy chỉnh
                </label>
            </div>

            <div id="meterTimeCustomArea" style="display:none;">
...
function initMeterTimeSetting() {
    ...
    loadMeterTime();
    $("input[name='meterTimeMode']")
        .off("change.meterTime")
        .on("change.meterTime", function () {
            const mode = $("input[name='meterTimeMode']:checked").val();

            if (mode === "custom") {
                loadMeterTime();
                $("#meterTimeCustomArea").show();
            } else {
                $("#meterTimeCustomArea").hide();
            }
    ...
    $("input[name='meterTimeMode'][value='server']").prop("checked", true);
    $("#meterTimeCustomArea").hide();

    updateMeterTimeHidden();
}
function loadMeterTime() {
    const now = new Date();
...
async function ThucHien_Ghi() {
    ...
    if (eventKey === "TOU_SEASON_ACTIVE_TIME") {
        ...
        const value = buildFutureSeasonActiveTimeValue();
        ...
        const meta = getFutureMetaByEventKey(eventKey);
        if (!validateFutureTariffSeasonBeforeSave(meta)) {
            return;
        }
        ...
        updateFutureTariffActiveDateTimeHidden();

        const el = $("#futureTariffActiveDateTime");
        const value = el.val();

        if (!/^\(\d{10}\)$/.test(value)) {
            toastr.error("Vui lòng chọn đúng thời gian kích hoạt biểu giá tương lai", "Thông báo");
            return;
        }

        obisMap[el.data("obis")] = value;
    }
    else if (eventKey === "SPECIAL_DAY_SETTING") {
        updateSpecialDayHidden();

        const el = $("#specialDaySetting");
        const value = el.val();

        if (!value) {
            toastr.error("Vui lòng chọn ít nhất một ngày đặc biệt", "Thông báo");
            return;
        }
        obisMap[el.data("obis")] = value;
    }
    else if (eventKey === "SPECIAL_DAY_TARIFF_TIME") {
        updateSpecialDayTariffHidden();

        const el = $("#specialDayTariffTime");

        const value = el.val();

        if (!value) {
            toastr.error(
                "Vui lòng chọn bảng biểu giá ngày đặc biệt",
                "Thông báo"
            );
            return;
        }
        ...
        if (!/^\(([1-8])\)$/.test(value)) {
            toastr.error(
                "Sai định dạng bảng biểu giá ngày đặc biệt",
                "Thông báo"
            );
            return;
        }
        ...
        if (mode === "server") {
            ...
            if (!dd || !mm || !yyyy || !hh || !mi || !ss) {

                toastr.error(
                    "Vui lòng chọn đầy đủ thời gian công tơ",
                    "Thông báo"
                );
            ...
            const value = `(${yy}${mm}${dd}${hh}${mi}${ss})`;

            // validate YYMMDDHHmmss
            if (!/^\(\d{12}\)$/.test(value)) {

                toastr.error(
                    "Sai định dạng thời gian công tơ",
                    "Thông báo"
                );
            ...
            $("#msg").append(`<p style="color:green">✔ Cài thời gian tùy chỉnh:${dd}/${mm}/${yyyy} ${hh}:${mi}:${ss}</p>`);
        }
    }
    else {
    ...
    try {
        ...
        const viNameMap = {
            ...
            // BIỂU GIÁ TƯƠNG LAI
            "0.0.13.0.10": "Thời gian kích hoạt của từng mùa",

            "0.0.13.1.11": "Định nghĩa ngày làm việc / ngày nghỉ mùa 1",
            "0.0.13.1.12": "Định nghĩa ngày làm việc / ngày nghỉ mùa 2",
            "0.0.13.1.13": "Định nghĩa ngày làm việc / ngày nghỉ mùa 3",
            "0.0.13.1.14": "Định nghĩa ngày làm việc / ngày nghỉ mùa 4",

            "0.0.13.2.11": "Thời gian chuyển biểu giá ngày làm việc mùa 1",
            "0.0.13.2.12": "Thời gian chuyển biểu giá ngày cuối tuần mùa 1",
            "0.0.13.2.13": "Thời gian chuyển biểu giá ngày làm việc mùa 2",
            "0.0.13.2.14": "Thời gian chuyển biểu giá ngày cuối tuần mùa 2",
            "0.0.13.2.15": "Thời gian chuyển biểu giá ngày làm việc mùa 3",
            "0.0.13.2.16": "Thời gian chuyển biểu giá ngày cuối tuần mùa 3",
            "0.0.13.2.17": "Thời gian chuyển biểu giá ngày làm việc mùa 4",
            "0.0.13.2.18": "Thời gian chuyển biểu giá ngày cuối tuần mùa 4",
        };

        const isTariffConfig = eventKey === "TOU_SEASON_ACTIVE_TIME" || eventKey === "FUTURE_TOU_SEASON_ACTIVE_TIME" || eventKey === "SPECIAL_DAY_TARIFF_TIME" || eventKey === "FUTURE_TARIFF_ACTIVE_DATETIME" || eventKey === "SPECIAL_DAY_SETTING" || eventKey === "METER_TIME_SETTING" || (eventKey && eventKey.startsWith("TOU_TARIFF_SEASON_")) || (eventKey && eventKey.startsWith("FUTURE_TARIFF_SEASON_"));
        ...
        for (const obis in obisMap) {
            ...
            const viName = el?.dataset?.name ?? viNameMap?.[obis] ?? (obis === "server" ? "Thời gian công tơ" : obis);
    ...
    for (let i = 0; i < obisList.length; i++) {
        ...
        try {
            ...
            if (eventKey === "TOU_SEASON_ACTIVE_TIME") {
                ...
                if (raw) {
                    $("#msg").append(`<p style="color:green">✔ Đọc thành công: ${viName} = ${raw || "null"}</p>`);
                } else {
                    $("#msg").append(`<p style="color:red">❌ Lỗi đọc: ${viName}</p>`);
                }
                ...
                const raw = getObisValueFromReadResponse(value, obis);
                parseFutureSeasonActiveTimeFromRead(raw);
                if (raw) {
                    $("#msg").append(`<p style="color:green">✔ Đọc thành công: ${viName} = ${Object.values(value.data)[0] || "null"}</p>`);
                } else {
                    $("#msg").append(`<p style="color:red">❌ Lỗi đọc: ${viName}</p>`);
                ...
                const season = getFutureTariffSeasonNo(eventKey);
                const meta = getFutureTariffMetaBySeason(season);
                ...
                const raw = getObisValueFromReadResponse(value, obis);
                if (raw) {
                    fillFutureTariffSeasonReadValue(season, value, obis);

                    $("#msg").append(`<p style="color:green">✔ Đọc thành công: ${nameMap[obis] || viName} = ${raw}</p>`);
                } else {
                    $("#msg").append(`<p style="color:red">❌ Lỗi đọc: ${nameMap[obis] || viName}</p>`);
                }
            }
            else if (eventKey === "FUTURE_TARIFF_ACTIVE_DATETIME") {
                const raw = getObisValueFromReadResponse(value, obis);

                fillFutureTariffActiveDateTime(raw);
...
//==================GHI LẠI CẤU HÌNH NHỮNG OBIS BỊ LỖI==============================//
...
// ================= BIỂU GIÁ TƯƠNG LAI / DỰ PHÒNG =================//

let futureSeasonDays = {
    1: [],
    2: [],
    3: [],
    4: []
...
const FUTURE_TARIFF_SEASON_META = {
    FUTURE_TARIFF_SEASON_1: {
        season: 1,
        dayId: "futuredinhnghiangaylamviecmua1",
        dayObis: "0.0.13.1.11",
        workId: "futurecaidatthoigianbieugiangaylamviecmua1",
        workObis: "0.0.13.2.11",
        weekendId: "futurecaidatthoigianbieugiangaycuoituanmua1",
        weekendObis: "0.0.13.2.12"
    },
    FUTURE_TARIFF_SEASON_2: {
        season: 2,
        dayId: "futuredinhnghiangaylamviecmua2",
        dayObis: "0.0.13.1.12",
        workId: "futurecaidatthoigianbieugiangaylamviecmua2",
        workObis: "0.0.13.2.13",
        weekendId: "futurecaidatthoigianbieugiangaycuoituanmua2",
        weekendObis: "0.0.13.2.14"
    },
    FUTURE_TARIFF_SEASON_3: {
        season: 3,
        dayId: "futuredinhnghiangaylamviecmua3",
        dayObis: "0.0.13.1.13",
        workId: "futurecaidatthoigianbieugiangaylamviecmua3",
        workObis: "0.0.13.2.15",
        weekendId: "futurecaidatthoigianbieugiangaycuoituanmua3",
        weekendObis: "0.0.13.2.16"
    },
    FUTURE_TARIFF_SEASON_4: {
        season: 4,
        dayId: "futuredinhnghiangaylamviecmua4",
        dayObis: "0.0.13.1.14",
        workId: "futurecaidatthoigianbieugiangaylamviecmua4",
        workObis: "0.0.13.2.17",
        weekendId: "futurecaidatthoigianbieugiangaycuoituanmua4",
        weekendObis: "0.0.13.2.18"
    }
};

function renderFutureSeasonActiveTime(cfg) {
    return `
        <div class="season-box future-season-box">
            <h4>Biểu giá tương lai - Cài thời gian kích hoạt từng mùa</h4>

            <div class="season-toolbar">
                <label style="margin-top:7px;">Mùa:</label>
                <select id="futureSeasonSelect" class="form-control">
                    <option value="1">Mùa 1</option>
                    <option value="2">Mùa 2</option>
                    <option value="3">Mùa 3</option>
                    <option value="4">Mùa 4</option>
                </select>

                <label style="margin-top:7px;">Ngày:</label>
                <select id="futureDaySelect" class="form-control"></select>

                <label style="margin-top:7px;">Tháng:</label>
                <select id="futureMonthSelect" class="form-control"></select>

                <button type="button" class="btn btn-primary" onclick="addFutureSeasonDay()">
                    Thêm
                </button>
            </div>

            <div id="futureSeasonList" class="season-list"></div>
...
function initFutureSeasonActiveTime() {
    $("#futureDaySelect").empty();
    $("#futureMonthSelect").empty();

    for (let d = 1; d <= 31; d++) {
        const val = String(d).padStart(2, "0");
        $("#futureDaySelect").append(`<option value="${val}">${val}</option>`);
    }

    for (let m = 1; m <= 12; m++) {
        const val = String(m).padStart(2, "0");
        $("#futureMonthSelect").append(`<option value="${val}">${val}</option>`);
    }

    $("#futureSeasonSelect,#futureMonthSelect")
        .off("change.futureSeason")
        .on("change.futureSeason", refreshFutureAvailableDays);

    renderFutureSeasonList();
    refreshFutureAvailableDays();
    updateFutureSeasonHiddenValue();
}

function addFutureSeasonDay() {
    const season = $("#futureSeasonSelect").val();
    const day = $("#futureDaySelect").val();
    const month = $("#futureMonthSelect").val();

    if (!season || !day || !month) {
        toastr.warning("Vui lòng chọn mùa, ngày và tháng", "Thông báo");
        return;
    }

    const key = `${day}/${month}`;

    if (futureSeasonDays[season].some(x => x.key === key)) {
        toastr.warning(`Ngày ${key} đã có trong mùa ${season}`, "Thông báo");
        refreshFutureAvailableDays();
        return;
    }

    futureSeasonDays[season].push({
        key,
        day,
        month,
        season,
        value: `${day}${month}${String(season).padStart(2, "0")}`
    });

    futureSeasonDays[season].sort((a, b) => Number(a.month + a.day) - Number(b.month + b.day));
...
function removeFutureSeasonDay(season, key) {
    futureSeasonDays[season] = futureSeasonDays[season].filter(x => x.key !== key);
...
function refreshFutureAvailableDays() {
    const season = $("#futureSeasonSelect").val();
    const month = $("#futureMonthSelect").val();

    const used = new Set(
        (futureSeasonDays[season] || [])
            .filter(x => x.month === month)
            .map(x => x.day)
    );

    $("#futureDaySelect option").each(function () {
        $(this).prop("disabled", used.has($(this).val()));
    });

    if ($("#futureDaySelect option:selected").prop("disabled")) {
        const first = $("#futureDaySelect option:not(:disabled)").first().val();
        if (first) $("#futureDaySelect").val(first);
    }

    $("#futureDaySelect").trigger("change.select2");
}

function renderFutureSeasonList() {
    let html = "";
    ...
    for (let season = 1; season <= 4; season++) {
        ...
        if (!futureSeasonDays[season].length) {
            html += `<div class="text-muted">Chưa chọn ngày</div>`;
        } else {
            futureSeasonDays[season].forEach(item => {
                html += `
                    <span class="season-tag">
                        ${item.key}
                        <button type="button" onclick="removeFutureSeasonDay(${season}, '${item.key}')">×</button>
                    </span>
                `;
            });
        }
    ...
    $("#futureSeasonList").html(html);
}

function updateFutureSeasonHiddenValue() {
    const result = [];

    Object.keys(futureSeasonDays).forEach(season => {
        futureSeasonDays[season].forEach(item => {
            result.push(`(${item.value})`);
        });
    });

    $("#futureSeasonActiveTime").val(result.join(""));
}

function renderFutureTariffSeason(meta) {
    const days = [
        { val: 2, text: "Thứ 2" },
        { val: 3, text: "Thứ 3" },
        { val: 4, text: "Thứ 4" },
    ...
    return `
        <div class="tariff-season-box future-tariff-box">
            <div class="tariff-section">
                <h4>1. Định nghĩa ngày làm việc / ngày nghỉ mùa ${meta.season}</h4>
                    ...
                    <tbody>
                        ${days.map(d => `
                            <tr>
                                ...
                                <td class="text-center">
                                    <input type="radio"
                                           name="future_season${meta.season}_day_${d.val}"
                                           value="1"
                                           class="future-tariff-day"
                                           ${d.val <= 6 ? "checked" : ""}>
                                </td>
                                <td class="text-center">
                                    <input type="radio"
            ...
            <div class="tariff-section">
                ...
                <button type="button" class="btn btn-primary btn-sm" onclick="addTariffTimeRow('workday')">
                    + Thêm mốc giờ
                </button>
            ...
            <div class="tariff-section">
                ...
                <button type="button" class="btn btn-primary btn-sm" onclick="addTariffTimeRow('weekend')">
                    + Thêm mốc giờ
                </button>
...
function initFutureTariffSeason(meta) {
    window.currentTariffSeasonMeta = meta;
    window.isFutureTariff = true;

    $(".future-tariff-day")
        .off("change.futureTariff")
        .on("change.futureTariff", function () {
            updateFutureTariffSeasonHidden(meta);
        });
...
function buildFutureTariffDayTypeValue(season) {
    const values = [];

    for (let d = 2; d <= 8; d++) {
        const checked = $(`input[name="future_season${season}_day_${d}"]:checked`).val();
...
function updateFutureTariffSeasonHidden(meta) {
    if (!meta) return;

    $("#" + meta.dayId).val(buildFutureTariffDayTypeValue(meta.season));
    $("#" + meta.workId).val(buildTariffTimeValueForObis("workday"));
    $("#" + meta.weekendId).val(buildTariffTimeValueForObis("weekend"));
}

function getFutureMetaByEventKey(eventKey) {
    const season = Number(String(eventKey || "").replace("FUTURE_TARIFF_SEASON_", ""));
    const map = {
        1: {
            season: 1,
            dayId: "futuredinhnghiangaylamviecmua1",
...
function getFutureSeasonNo(eventKey) {
    return Number(String(eventKey).replace("FUTURE_TARIFF_SEASON_", ""));
}
function getFutureMetaBySeason(season) {
    const map = {
        1: {
            dayObis: "0.0.13.1.11",
            workObis: "0.0.13.2.11",
...
function parseFutureSeasonActiveTimeFromRead(rawValue) {
    let arr = [];
    ...
    arr.forEach(function (raw) {
        ...
        if (!futureSeasonDays[season]) return;
    ...
    renderFutureSeasonList();
    refreshFutureAvailableDays();
    updateFutureTariffSeasonHidden();
}


function getFutureTariffSeasonNo(eventKey) {
    return Number(String(eventKey || "").replace("FUTURE_TARIFF_SEASON_", ""));
}

function getFutureTariffMetaBySeason(season) {
    return FUTURE_TARIFF_SEASON_META[`FUTURE_TARIFF_SEASON_${season}`] || null;
}
function fillFutureTariffSeasonReadValue(season, value, obis) {
    const meta = getFutureTariffMetaBySeason(season);
    if (!meta) return;
    ...
    if (obis === meta.dayObis && dayArr.length) {
        fillFutureTariffDayType(season, dayArr[0]);
    }

    if (obis === meta.workObis && workArr.length) {
        fillTariffTimeRowsFromObis("workday", parseTariffRowsFromArray(workArr));
    }

    if (obis === meta.weekendObis && weekendArr.length) {
        fillTariffTimeRowsFromObis("weekend", parseTariffRowsFromArray(weekendArr));
    }

    updateFutureTariffSeasonHidden(meta);
}

function fillFutureTariffDayType(season, value) {
    const arr = String(value || "").split(";");
    ...
    for (let i = 0; i < 7; i++) {
        ...
        // UI mới
        $(`input[name="future_season${season}_day_${day}"][value="${val}"]`).prop("checked", true);

    }
}

function fillTariffTimeRowsFromObis(type, rows) {
    if (!rows || !rows.length) return;
...
function clearTariffSeasonUI() {

    // clear workday/weekend rows
    $("#workdayTariffRows").empty();
    $("#weekendTariffRows").empty();

    // clear hidden values
    $([
        "#dinhnghiangaylamviecmua1",
        "#dinhnghiangaylamviecmua2",
        "#dinhnghiangaylamviecmua3",
        "#dinhnghiangaylamviecmua4",

        "#caidatthoigianbieugiangaylamviecmua1",
        "#caidatthoigianbieugiangaylamviecmua2",
        "#caidatthoigianbieugiangaylamviecmua3",
        "#caidatthoigianbieugiangaylamviecmua4",

        "#caidatthoigianbieugiangaycuoituanmua1",
        "#caidatthoigianbieugiangaycuoituanmua2",
        "#caidatthoigianbieugiangaycuoituanmua3",
        "#caidatthoigianbieugiangaycuoituanmua4",

        "#futuredinhnghiangaylamviecmua1",
        "#futuredinhnghiangaylamviecmua2",
        "#futuredinhnghiangaylamviecmua3",
        "#futuredinhnghiangaylamviecmua4",

        "#futurecaidatthoigianbieugiangaylamviecmua1",
        "#futurecaidatthoigianbieugiangaylamviecmua2",
        "#futurecaidatthoigianbieugiangaylamviecmua3",
        "#futurecaidatthoigianbieugiangaylamviecmua4",

        "#futurecaidatthoigianbieugiangaycuoituanmua1",
        "#futurecaidatthoigianbieugiangaycuoituanmua2",
        "#futurecaidatthoigianbieugiangaycuoituanmua3",
        "#futurecaidatthoigianbieugiangaycuoituanmua4"
    ].join(",")).val("");
...
function buildFutureSeasonActiveTimeValue() {
    const result = [];

    Object.keys(futureSeasonDays).forEach(function (season) {
        futureSeasonDays[season].forEach(function (item) {
            const dd = String(item.day).padStart(2, "0");
            const mm = String(item.month).padStart(2, "0");
            const ss = String(season).padStart(2, "0");
...
function validateFutureTariffSeasonBeforeSave(meta) {
    if (!meta) {
        toastr.error("Không xác định được mùa biểu giá", "Thông báo");
        return false;
    }
...
function renderSpecialDaySetting(cfg) {
    return `
        <div class="season-box special-day-box">
            <h4>Ngày đặc biệt</h4>

            <div class="season-toolbar">
                <label style="margin-top:7px;">Ngày:</label>
                <select id="specialDaySelect" class="form-control"></select>

                <label style="margin-top:7px;">Tháng:</label>
                <select id="specialMonthSelect" class="form-control"></select>

                <button type="button" class="btn btn-primary" onclick="addSpecialDay()">
                    Thêm
                </button>
            </div>

            <div id="specialDayList" class="season-list"></div>
...
function initSpecialDaySetting() {
    $("#specialDaySelect").empty();
    $("#specialMonthSelect").empty();

    for (let d = 1; d <= 31; d++) {
        const val = String(d).padStart(2, "0");
        $("#specialDaySelect").append(`<option value="${val}">${val}</option>`);
    }

    for (let m = 1; m <= 12; m++) {
        const val = String(m).padStart(2, "0");
        $("#specialMonthSelect").append(`<option value="${val}">${val}</option>`);
    }
...
function addSpecialDay() {
    const day = $("#specialDaySelect").val();
    const month = $("#specialMonthSelect").val();

    const key = `${day}/${month}`;
    const value = `${day}${month}`;

    if (specialDays.some(x => x.value === value)) {
        toastr.warning(`Ngày ${key} đã tồn tại`, "Thông báo");
        return;
    }

    specialDays.push({ key, value });

    specialDays.sort((a, b) => Number(a.value.substring(2, 4) + a.value.substring(0, 2)) -
        Number(b.value.substring(2, 4) + b.value.substring(0, 2)));

    renderSpecialDayList();
    updateSpecialDayHidden();
}

function removeSpecialDay(value) {
    specialDays = specialDays.filter(x => x.value !== value);
...
function renderSpecialDayList() {
    ...
    if (!specialDays.length) {
        html = `<div class="text-muted">Chưa chọn ngày đặc biệt</div>`;
    } else {
        specialDays.forEach(item => {
            html += `
                <span class="season-tag">
                    ${item.key}
                    <button type="button" onclick="removeSpecialDay('${item.value}')">×</button>
                </span>
            `;
        });
    }

    $("#specialDayList").html(html);
}

function updateSpecialDayHidden() {
    const values = specialDays.map(x => x.value);
    $("#specialDaySetting").val(`(${values.join(";")})`);
}
function fillSpecialDaySetting(raw) {
    ...
    specialDays = arr.map(v => ({
        value: v,
        key: `${v.substring(0, 2)}/${v.substring(2, 4)}`
    }));
...
//============================================//
const SPECIAL_DAY_TARIFF_OPTIONS = [
    { value: "1", text: "Thời gian chuyển biểu giá ngày làm việc mùa 1" },
    { value: "2", text: "Thời gian chuyển biểu giá ngày cuối tuần mùa 1" },
    { value: "3", text: "Thời gian chuyển biểu giá ngày làm việc mùa 2" },
    { value: "4", text: "Thời gian chuyển biểu giá ngày cuối tuần mùa 2" },
    { value: "5", text: "Thời gian chuyển biểu giá ngày làm việc mùa 3" },
    { value: "6", text: "Thời gian chuyển biểu giá ngày cuối tuần mùa 3" },
    { value: "7", text: "Thời gian chuyển biểu giá ngày làm việc mùa 4" },
    { value: "8", text: "Thời gian chuyển biểu giá ngày cuối tuần mùa 4" }
];

function buildSpecialDayTariffOptions(selected = "1") {
    return SPECIAL_DAY_TARIFF_OPTIONS.map(item => `
...
function renderSpecialDayTariffTime() {
    return `
        <div class="tariff-season-box special-day-tariff-box">
            <div class="tariff-section">
                <h4>Thời gian chuyển biểu giá của ngày đặc biệt</h4>

                <div class="form-row">
                    <label>Bảng biểu giá áp dụng</label>
                    <select id="specialDayTariffSelect" class="form-control">
                        ${buildSpecialDayTariffOptions("1")}
                    </select>
                ...
                <input type="hidden"
                       id="specialDayTariffTime"
                       data-obis="0.0.13.9.2"
                       data-name="Thời gian chuyển biểu giá của ngày đặc biệt">
            </div>
        </div>
    `;
}

function initSpecialDayTariffTime() {
    $("#specialDayTariffSelect")
        .off("change.specialDayTariff")
        .on("change.specialDayTariff", updateSpecialDayTariffHidden);

    updateSpecialDayTariffHidden();
}

function updateSpecialDayTariffHidden() {
    const value = $("#specialDayTariffSelect").val() || "1";
    $("#specialDayTariffTime").val(`(${value})`);
}
function fillSpecialDayTariffTime(raw) {
    ...
    $("#specialDayTariffSelect").val(String(value));

    updateSpecialDayTariffHidden();
}
function validateSpecialDayTariffRowsUi() {
    const value = $("#specialDayTariffSelect").val();

    if (!value) {
        toastr.error("Vui lòng chọn bảng biểu giá ngày đặc biệt", "Thông báo");
        return false;
    }
...
//================================FUTURE_TARIFF_ACTIVE_DATETIME===================
function renderFutureTariffActiveDateTime() {
    return `
        <div class="meter-time-box">
            <h4>Cài thời gian kích hoạt biểu giá tương lai</h4>

            <div class="meter-time-grid">
                <div>
                    <label>Ngày</label>
                    <select id="futureTariffActiveDay" class="form-control"></select>
                </div>

                <div>
                    <label>Tháng</label>
                    <select id="futureTariffActiveMonth" class="form-control"></select>
                </div>

                <div>
                    <label>Năm</label>
                    <select id="futureTariffActiveYear" class="form-control"></select>
                </div>

                <div>
                    <label>Giờ</label>
                    <select id="futureTariffActiveHour" class="form-control"></select>
                </div>

                <div>
                    <label>Phút</label>
                    <select id="futureTariffActiveMinute" class="form-control"></select>
                </div>
            </div>

            <input type="hidden"
                   id="futureTariffActiveDateTime"
                   data-obis="0.0.13.6.0"
                   data-name="Cài thời gian kích hoạt biểu giá tương lai">
        </div>
    `;
}

function initFutureTariffActiveDateTime() {
    const now = new Date();

    fillSelectNumber("#futureTariffActiveDay", 1, 31, now.getDate(), 2);
    fillSelectNumber("#futureTariffActiveMonth", 1, 12, now.getMonth() + 1, 2);
    fillSelectNumber("#futureTariffActiveYear", now.getFullYear(), now.getFullYear() + 10, now.getFullYear(), 4);
    fillSelectNumber("#futureTariffActiveHour", 0, 23, now.getHours(), 2);
    fillSelectNumber("#futureTariffActiveMinute", 0, 59, now.getMinutes(), 2);

    $("#futureTariffActiveDay,#futureTariffActiveMonth,#futureTariffActiveYear,#futureTariffActiveHour,#futureTariffActiveMinute")
        .off("change.futureTariffActive")
        .on("change.futureTariffActive", updateFutureTariffActiveDateTimeHidden);

    updateFutureTariffActiveDateTimeHidden();
}

function updateFutureTariffActiveDateTimeHidden() {
    const dd = $("#futureTariffActiveDay").val();
    const mm = $("#futureTariffActiveMonth").val();
    const yyyy = $("#futureTariffActiveYear").val();
    const hh = $("#futureTariffActiveHour").val();
    const mi = $("#futureTariffActiveMinute").val();

    const yy = String(yyyy).slice(-2);

    $("#futureTariffActiveDateTime").val(`(${yy}${mm}${dd}${hh}${mi})`);
}
function fillSelectNumber(selector, from, to, selected, padLength) {
    let html = "";
    ...
    for (let i = from; i <= to; i++) {
        const value = String(i).padStart(padLength, "0");
        html += `<option value="${value}" ${Number(i) === Number(selected) ? "selected" : ""}>${value}</option>`;
    }

    $(selector).html(html);
}
function fillFutureTariffActiveDateTime(raw) {
    const text = String(raw || "").trim();
    const clean = text.replace(/[()]/g, "");

    if (!/^\d{10}$/.test(clean)) {
        console.log("Sai format thời gian kích hoạt biểu giá tương lai:", raw);
        return;
    }
    ...
    const hh = clean.substring(6, 8);
    const mi = clean.substring(8, 10);

    const yyyy = Number(yy) >= 70 ? `19${yy}` : `20${yy}`;

    $("#futureTariffActiveDay").val(dd);
    $("#futureTariffActiveMonth").val(mm);
    $("#futureTariffActiveYear").val(yyyy);
    $("#futureTariffActiveHour").val(hh);
    $("#futureTariffActiveMinute").val(mi);
