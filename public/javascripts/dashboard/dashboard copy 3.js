...
//     // console.log("[DASHBOARD] received dashboard-realtime:", data);

//     if (!data) return;

//     updateRealtimeValue("voltage", data.voltage, 2);
//     updateRealtimeValue("current", data.current, 2);
//     updateRealtimeValue("power", data.power, 2);
//     updateRealtimeValue("frequency", data.frequency, 2);
//     updateRealtimeValue("pf", data.pf, 2);
//   });
// } else {
//   console.error("[DASHBOARD] socket.io client not found");
// }

// function updateRealtimeValue(field, value, digits) {
//   const el = document.querySelector(`[data-field="${field}"]`);

//   if (!el) {
//     //  console.warn("[DASHBOARD] missing element:", field);
//     return;
//   }
...
//   el.classList.remove("iec-flash");
//   void el.offsetWidth;
//   el.classList.add("iec-flash");
// }

// dashboard
$(document).ready(function () {

  tt_nhamay = localStorage.getItem("id_nhamay");
  var chid_code = tt_nhamay.id_nhamay;
 
  load_bdpt(chid_code);
  loadDuLieuSongHai_KH(chid_code);
  f_canhbao();
...
// sóng hài
function loadDuLieuSongHai_KH(chid_code) {
  ...
  DulieuSongHai.SoCongTo = id_nhamay === "1" ? "2527990001" : "2527990002";
...
async function f_canhbao() {
  ...
  try {
    ...
    console.log("data cảnh báo", data);
    var xxx = data;
  } catch (err) {
...
async function load_bdpt() {
  ...
  try {
    ...
    console.error("Lỗi load_bdpt:", err);
...
//   const c = document.querySelector("#loadChart");
//   if (!c || !feeders.length) return;
...
//   const dataRows = feeders.slice().reverse();

//   const series = [
//     { key: "pgiao", label: "P giao", color: "#ffbf35" },
//     { key: "pnhan", label: "P nhận", color: "#67e247" },
//     { key: "qgiao", label: "Q giao", color: "#4db7ff" },
//     { key: "qnhan", label: "Q nhận", color: "#ff4d6d" }
//   ];

//   const allValues = dataRows.flatMap(r => [
//     r.pgiao,
//     r.pnhan,
//     r.qgiao,
//     r.qnhan
...
//   // grid
//   ctx.strokeStyle = "rgba(120,170,210,.18)";
//   ctx.lineWidth = 1 * dpr;
...
//   function getX(i) {
//     if (dataRows.length === 1) return padding + chartW / 2;
//     return padding + i * chartW / (dataRows.length - 1);
//   }

//   function getY(value) {
//     return padding + (max - value) * chartH / (max - min);
//   }
...
//       const x = getX(i);
//       const y = getY(Number(row[s.key] || 0));
...
//   series.forEach((s, i) => {
//     const x = padding + i * 70 * dpr;
//     const y = 12 * dpr;

//     ctx.fillStyle = s.color;
//     ctx.fillRect(x, y, 10 * dpr, 3 * dpr);

//     ctx.fillStyle = "#dceeff";
//     ctx.fillText(s.label, x + 16 * dpr, y + 1 * dpr);
//   });
// }
