...
function getDatakhaibaotb(v_idthietbi, v_tencambien, v_giaothuc,v_type) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_IEC_104.KHAOBAO_CAMBIEN_IEC_104(:idthietbi,:tencambien,:giaothuc,:type,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        idthietbi: {
...
async function find(context) {
  ...
  if (context.tencambien) {
    idthietbi = context.idthietbi;
    tencambien = context.tencambien;
    giaothuc = context.giaothuc;
    type=context.type;
    var data = getDatakhaibaotb(idthietbi, tencambien,giaothuc,type);
    return data;
