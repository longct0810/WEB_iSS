...
function getDatakhaibaotb(v_parentID, v_tendanhmuc, v_loaitree, type) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_DANHMUC.P_THEM_DANHMUC(:parentID,:tendanhmuc,:loaitree,:type,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        parentID: {
...
async function find(context) {
  const { parentID, tendanhmuc, loaitree, type } = context;
  var data = getDatakhaibaotb(parentID, tendanhmuc, loaitree, type);
  return data;
}
