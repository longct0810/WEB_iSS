...
function getDatakhaibaotb(v_tenthietbi, v_ip, v_port, v_userid) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_IEC_104.KHAOBAO_THIETBI_IEC_104(:tenthietbi,:ip,:port,:userid,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        tenthietbi: {
...
async function find(context) {
  ...
  if (context.tenthietbi) {
    tenthietbi = context.tenthietbi;
    ip = context.ip;
    port = context.port;
    userid = context.userid;
    var data = getDatakhaibaotb(tenthietbi, ip, port,userid);
    return data;
