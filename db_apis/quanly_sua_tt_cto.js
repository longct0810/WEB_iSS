...
function sua_tt_diemdo(v_meterid, v_socongto, v_loaicongto, v_matkhaucongto, v_outstation, v_mataikhoan) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_QUANLY_DIEMDO.P_SUA_THONGTIN_CONGTO(:meterid,:socongto,:loaicongto,:matkhaucongto,:outstation,:mataikhoan,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        meterid: {
...
async function find(context) {
  const { meterid, socongto, loaicongto, matkhaucongto, outstation, mataikhoan } = context;

  var data = sua_tt_diemdo(meterid, socongto, loaicongto, matkhaucongto, outstation, mataikhoan);
  return data;
