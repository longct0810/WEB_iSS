...
function phanquyen_ganquyen(quyen,mataikhoan,mataikhoanthuchien) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_PHANQUYEN.P_SUA_PHAN_QUYEN(:quyen,:mataikhoan,:mataikhoanthuchien,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        quyen: {
...
async function find(context) {
  //console.log(context);
  var binds = {};
  var quyen = oracledb.STRING;
  var mataikhoan = oracledb.NUMBER;
  var mataikhoanthuchien=oracledb.NUMBER;
  quyen=JSON.stringify(JSON.parse(context.quyen));
  mataikhoan = context.mataikhoan;
  mataikhoanthuchien= context.mataikhoanthuchien;
  if (context.mataikhoan) {
    console.log(quyen);
    console.log("mataikhoan");
    console.log(mataikhoan);
 var data = phanquyen_ganquyen(quyen,mataikhoan,mataikhoanthuchien);

 return data;
  }else{
    return "Chưa nhập mã tài khoản"
  }
