...
function ds_tbchiase(v_meterid, v_mataikhoan) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_QUANLY_DIEMDO.P_THANHLY_DIEMDO(:m,:tk,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        m: {
...
async function find(context) {
  ...
  var taikhoan = oracledb.NUMBER;
  if (context.meterid) {
    meterid = context.meterid;
    taikhoan = context.taikhoan;
    var data = ds_tbchiase(meterid,taikhoan);
    return data;
