...
function getbaocaohoatdong(iddanhmuc, macanhbao, tungay, denngay) {
    return oracledb.getConnection()
        .then(function (conn) {
            return conn.execute(`BEGIN pkg_canhbao.p_layds_canhbao_matdien(:iddanhmuc,:macanhbao,:tungay,:denngay,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
                iddanhmuc: {
...
async function find(context) {
    var binds = {};
    var iddanhmuc = oracledb.STRING;
    var macanhbao = oracledb.STRING;
    var tungay = oracledb.STRING;
    var denngay = oracledb.STRING;
    if (context.iddanhmuc) {
        iddanhmuc = context.iddanhmuc;
        macanhbao = context.macanhbao;
        tungay = context.tungay;
        denngay = context.denngay;
        var data = getbaocaohoatdong(iddanhmuc, macanhbao, tungay, denngay);
