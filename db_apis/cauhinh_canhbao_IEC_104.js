...
function getDatakhaibaotb(v_userid) {
    return oracledb.getConnection()
        .then(function (conn) {
            return conn.execute(`BEGIN PKG_CAUHINH_CANHBAO_SCADA.P_LAYDS_CAUHINH_CANHBAO(:userid,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
                userid: {
...
async function find(context) {
    ...
    if (context.userid) {
        ...
        var data = getDatakhaibaotb(userid);
        return data;

    } else {
        return "Đăng nhập hệ thống trước khi sử dụng"
    }
