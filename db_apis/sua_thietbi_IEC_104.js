...
function getDatakhaibaotb(v_tenthietbi, v_ip, v_port, v_userid,v_idthietbi) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_IEC_104.SUA_KHAOBAO_THIETBI_IEC_104(:tenthietbi,:ip,:port,:userid,:idthietbi,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        tenthietbi: {
        ...
        .then((result) => {
          ...
          return consumeStream = new Promise((resolve, reject) => {
            queryStream.on('data', (row) => {
              resRows.push(row); //STORE ROWS IN TO BLANK ARRAYs
            });
...
async function find(context) {
  ...
  var port = oracledb.NUMBER;
  var userid=oracledb.NUMBER;
  var idthietbi=oracledb.NUMBER;
  if (context.idthietbi) {
    tenthietbi = context.tenthietbi;
    ip = context.ip;
    port = context.port;
    userid = context.userid;
    idthietbi=context.idthietbi;
    var data = getDatakhaibaotb(tenthietbi, ip, port,userid,idthietbi);
    return data;
