...
function f_lay_tsvh_din(v_imei, v_time) {
    return oracledb.getConnection()
        .then(function (conn) {
            return conn.execute(`BEGIN PKG_TSVH_DIN.P_LAY_TSVH_DIN(:v_imei, :v_time,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
                v_imei: {
...
async function find(context) {
    var binds = {};
    var v_imei = oracledb.STRING;
    var v_time = oracledb.STRING;

    v_imei = context.v_imei;
    v_time = context.v_time;
    var data = f_lay_tsvh_din(v_imei, v_time);
    return data;
