...
function dsthietbi_iec_104(ma_ioa, loaiioa, idthietbi, type) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_QL_IOA.P_CAPNHAT_LOAI_THIETBI_IOA(:ma_ioa,:loaiioa,:idthietbi,:type,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        ma_ioa: {
...
async function find(context) {
  console.log(context);
  var binds = {};
  var ma_ioa = oracledb.NUMBER;
  var loai_ioa = oracledb.NUMBER;
  var id_thietbi = oracledb.STRING;
  var type = oracledb.STRING;
  if (context.ma_ioa && context.loai_ioa && context.id_thietbi) {
    id_thietbi = context.id_thietbi;
    ma_ioa = context.ma_ioa;
    loai_ioa = context.loai_ioa;
    type = context.type;
    var data = dsthietbi_iec_104(ma_ioa, loai_ioa, id_thietbi, type);
    console.log("data");
    console.log(data);
    return data;

  } else {
    return "Trường IOA, Loại IOA, id thiết bị bắt buộc nhập"
  }
