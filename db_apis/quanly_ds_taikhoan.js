...
function ds_tbchiase(v_mataikhoan) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_TAIKHOAN.P_LAYDS_TAIKHOAN(:t,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
        t: {
...
async function find(context) {
  console.log(context);
  var binds = {};
  var mataikhoan = oracledb.STRING;
  if (context.mataikhoan || context.mataikhoan==0) {
    mataikhoan = context.mataikhoan;
    var data = ds_tbchiase(mataikhoan);
    return data;

  } else {
    return "Mã tài khoản không đúng"
  }
