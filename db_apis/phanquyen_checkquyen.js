...
function phanquyen_ganquyen(mataikhoan,machucnang) {
  return oracledb.getConnection()
    .then(function (conn) {
      return conn.execute(`BEGIN PKG_PHANQUYEN.P_CHECK_QUYEN(:mataikhoan,:machucnang,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE       
        mataikhoan: {
...
async function find(context) {
  ...
  var machucnang = oracledb.NUMBER;
  var type=oracledb.STRING;  
  mataikhoan = context.mataikhoan;
  machucnang= context.machucnang;
 
  if (context.mataikhoan || context.mataikhoan==0 ) {
     var data = phanquyen_ganquyen(mataikhoan,machucnang);
    return data;
  }else{
