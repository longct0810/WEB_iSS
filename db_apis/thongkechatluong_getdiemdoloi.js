...
function laydsdiemdoloi(danhmucid, ngay, starttime, currenttime, mataikhoan, phanbiet) {
    return oracledb.getConnection()
        .then(async (conn) => {
            try {
                // Gọi procedure Oracle
                const result = await conn.execute(
                    `BEGIN PKG_THONGKE_CHATLUONG_HT.P_LAY_DS_DIEMDO_LOI(
                        :danhmucid, :ngay, :starttime, :currenttime, :mataikhoan,:phanbiet, :CV_1
                    ); END;`,
                    {
                        ...
                        phanbiet: { type: oracledb.STRING, dir: oracledb.BIND_IN, val: phanbiet },
...
async function find(context) {
    ...
    var phanbiet = oracledb.STRING;
    danhmucid = context.danhmucid;
    ngay = context.ngay;
    starttime = context.starttime;
    currenttime = context.currenttime;
    mataikhoan = context.mataikhoan;
    phanbiet = context.phanbiet;
    var data = laydsdiemdoloi(danhmucid, ngay, starttime, currenttime, mataikhoan, phanbiet);
    return data;
