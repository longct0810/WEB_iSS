...
function getKhachHang(danhmucid, mataikhoan) {
    return oracledb.getConnection()
        .then(function (conn) {
            return conn.execute(`BEGIN PKG_CAUHINH.P_DS_KHACHHANG_CAUHINHTHIETBI(:danhmucid,:mataikhoan,:CV_1); END;`, { // EXECUTE ORACLE PROCEDURE
                danhmucid: {
...
async function find(context) {
    let danhmucid = oracledb.STRING;
    let mataikhoan = oracledb.STRING;
    danhmucid = context.danhmucid;
    mataikhoan = context.mataikhoan;
    var data = getKhachHang(danhmucid, mataikhoan);
    return data;
